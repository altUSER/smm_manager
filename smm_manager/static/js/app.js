...
Vue.createApp(AppPub).mount('#app_pub')
...
const AppDel = {
  ...
  methods: {
    async getPosts() {
      try {
        const {data} = await axios.get(`http://127.0.0.1:8000/get_json_delayed_posts?page=${this.page}&limit=${this.limit}`)
        this.posts.push(...data)
      } catch (e) {
        console.log(e.message)
      }
...
Vue.createApp(AppDel).mount('#app_del')